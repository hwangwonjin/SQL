...
create user 'java2_admin'@'%' identified by '1234';
grant all privileges on `java2_Bank`.* to 'java2_admin'@'%';
flush privileges;
...
insert into `bank_account` values('101-12-1002','S2','정기적금예금','830513-2000003',1020000,'2011-05-13');
...
#실습2-14
select * from `bank_transaction` where `t_dist` like 2 
order by `t_amount` desc;

#실습2-15
select sum(`t_amount`) as `임금총합`, avg(`t_amount`) as `임금평균` 
...
#실습2-20
select count(case when`t_dist`=1 then 1 end) as `입금 건수`, count(case when `t_dist`=2 then 2 end) as `출금 건수`,
count(case when `t_dist`=3 then 3 end) as `조회 건수`
from `bank_transaction`;

#실습2-21
select `t_dist`, case when `t_dist`= 1 then '입금'
 when `t_dist` = 2 then '출금' when `t_dist` = 3 then '조회'
...
select 
 ...
 from `bank_account` as a
join `bank_transaction` as b on a.a_no = b.t_a_no
